...
async function insertOne(objCustData) {
  return await quiltQuery(`INSERT INTO customers SET ?`, [objCustData]);
}
...
async function updateOne(cintID, objCustData) {
  return await quiltQuery(`UPDATE customers SET ? WHERE CustomerID = ?`, [
    objCustData,
    cintID,
  ]);
}
...
async function deleteOne(cintID) {
  return await quiltQuery(`DELETE FROM customers WHERE CustomerID = ?`, [
    cintID,
  ]);
}
