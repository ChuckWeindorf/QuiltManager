...
async function getOne(cintID) {
  return await quiltQuery(
    "SELECT * FROM sales_artifacts WHERE SaleArtifactID = ?",
    [cintID]
  );
}
...
async function getOneWorkOrder(cintID) {
  return await quiltQuery(
    "SELECT artifacts.ArtType, artifacts.Category, artifacts.FileName FROM sales_artifacts, artifacts " +
...
async function insertOne(objSalesArtifactsData) {
  return await quiltQuery(`INSERT INTO sales_artifacts SET ?`, [
...
module.exports = {getAll, getOne, getOneWorkOrder, insertOne, updateOne, deleteOne}