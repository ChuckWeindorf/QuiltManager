...
async function getOne(cintID) {
  return await quiltQuery("SELECT * FROM artifacts WHERE ArtifactID = ?", [
    cintID,
  ]);
}
...
async function getCatList(cstrCat) {
  ...
  return await quiltQuery(
    `SELECT * FROM artifacts WHERE arttype = "Pattern" and category = ?`,
    [cstrCat]
  );
}
...
async function insertOne(objArtifactsData) {
  return await quiltQuery(`INSERT INTO artifacts SET ?`, [objArtifactsData]);
}
...
async function updateOne(cintID, objArtifactsData) {
  return await quiltQuery(`UPDATE artifacts SET ? WHERE ArtifactID = ?`, [
    objArtifactsData,
    cintID,
  ]);
}
...
async function deleteOne(cintID) {
  return await quiltQuery(`DELETE FROM artifacts WHERE ArtifactID = ?`, [
    cintID,
  ]);
}
